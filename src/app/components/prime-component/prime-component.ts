...
@Component({
    ...
    imports: [CommonModule, FormsModule, ReactiveFormsModule, ToastModule, ToolbarModule, TableModule, IconFieldModule, InputIconModule, ButtonModule, DialogModule, FloatLabelModule, InputNumberModule, InputTextModule],
...
export class PrimeComponent {
    ...
    async checkIfPrimeExists(prime: Prime): Promise<boolean> {
        try {
    ...
    controle(): boolean {
        let controle = false;
        if (this.prime.id) {
            controle = this.listPrime.some((x) => (x.montant >= this.formGroup.get('montant')?.value || x.prime >= this.formGroup.get('prime')?.value) && x.id !== this.prime.id);
        } else {
            controle = this.listPrime.some((x) => x.montant >= this.formGroup.get('montant')?.value || x.prime >= this.formGroup.get('prime')?.value);
        }
        if (controle) {
            this.messageService.add({ severity: 'error', summary: this.msg.summary.labelError, detail: this.msg.components.prime.messageErrorSupOrEgal });
        }
        return controle;
    }
    ...
    async miseAjour(): Promise<void> {
        ...
        trvErreur = await this.checkIfPrimeExists(primeSearch);
