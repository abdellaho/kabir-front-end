...
export function getLastDayOfMonth(dateDebut: Date): Date {
...
export function getPrixVenteMin(stock: Stock): number {
    return stock.prixVentMin4 > 0 ? stock.prixVentMin4 : stock.prixVentMin3 > 0 ? stock.prixVentMin3 : stock.prixVentMin2 > 0 ? stock.prixVentMin2 : stock.prixVentMin1 > 0 ? stock.prixVentMin1 : stock.pvttc;
}

export function getRemiseMax(stock: Stock): number {
    return stock.remiseMax4 > 0 ? stock.remiseMax4 : stock.remiseMax3 > 0 ? stock.remiseMax3 : stock.remiseMax2 > 0 ? stock.remiseMax2 : stock.remiseMax1;
}
...
export function getAllInvalidFields(form: FormGroup | FormArray): { [key: string]: AbstractControl } {
    ...
    const recurse = (c: AbstractControl, path: string = '') => {
        if (c.invalid && (c.dirty || c.touched)) {
            // Optional: Only if touched/dirty
